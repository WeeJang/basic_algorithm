...
struct LCSCell{
public:
	LCSCell():backtrack_direction(Direction::Diagonal),common_length(0){
	}

	LCSCell(Direction backtrack_direction,uint64_t common_length):
		backtrack_direction(backtrack_direction),common_length(common_length){
	}
public:
...
*/
int LCS_finder(const std::wstring& str_1,const std::wstring& str_2,std::wstring& LCS_result){
...
int main(int argc,char** argv){
	std::wstring str1(L"你不假的哦好成功需一秒长");
	std::wstring str2(L"闷闷你非常好厉害长者长");
	...
	std::wstring result;
	size_t result_len = LCS_finder(str1,str2,result);	
	std::cout << "<<<<<<<<<<" << std::endl;
