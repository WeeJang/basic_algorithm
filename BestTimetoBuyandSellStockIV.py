...
class Solution(object):
	...
	D[k][j] = max{ D[k][j-1], prices[j] - prices[jj] + D[k-1][jj] for jj in range(0,j) }
                = max{ D[k][j-1], prices[j] + max{ D[k-1][jj] - prices[jj] for jj in range(0,j) } }
	D[0][j] = 0
	D[k][0] = 0 
        """
	...
	# 这种情况下(k > len(prices) / 2)任何向上的波动都能捕捉到,只要相邻两点间为非负，即可获得收益
	# 即 a[i] - [j] < 0 && i < j	
	profile = 0
	if k > len(prices) / 2:
	...
	D = [ [ 0 for j in range(len(prices)) ] for k_ in range(k+1) ]
	...
	for k_ in range(1,k+1):
		...
		for j in range(1,len(prices)):
			...
			这一步可以优化,因为重复计算了
			for jj in range(0,j):
				inner_p = D[k_ - 1][jj] - prices[jj]	
				if inner_p > inner_max:
					inner_max = inner_p
			"""
			inner_p = D[k_-1][j-1] - prices[j-1]
			if inner_p > inner_max:
					inner_max = inner_p				
			outter_p = prices[j] + inner_max
			if outter_p > outter_max:
				outter_max = outter_p
			D[k_][j] = outter_max
	#print D
	return D[k][len(prices)-1]	
...
solver = Solution()
print solver.maxProfit(k,prices)
